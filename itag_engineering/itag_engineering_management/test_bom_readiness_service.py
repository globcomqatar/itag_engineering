...
class TestBomReadinessService(FrappeTestCase):
	...
	def _make_bom(self, item_code, components=None, with_operation=True):
		if components is None:
			...
			# (a different stock Item than the assembly itself) so every
			# BOM built by this helper is a valid, insertable BOM; tests that
			# want to exercise a *different* incompleteness (missing
			# operations, missing Product Revision, etc.) still get that,
			# since this default component intentionally has no Engineering
			# Classification of its own either.
			component_item = frappe.db.get_value(
				"Item", {"is_stock_item": 1, "name": ["!=", item_code]}, "name"
			)
	...
	def test_hold_point_without_inspection_requirement_is_an_exception(self):
		...
		component_item = frappe.db.get_value("Item", {"is_stock_item": 1, "name": ["!=", item]}, "name")
		stock_uom = frappe.db.get_value("Item", component_item, "stock_uom")
		bom = frappe.get_doc(
			{
				"doctype": "BOM",
