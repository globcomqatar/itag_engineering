...
class TestECOService(FrappeTestCase):
	def setUp(self):
		...
		# frappe.db.delete() on the parent doctype only issues a raw DELETE
		# against `tabEngineering Approval Matrix` - it does NOT cascade to
		# the "Approval Matrix Discipline" child table (unlike doc.delete()).
		# Since this doctype autonames on rule_name (autoname:
		# "field:rule_name"), the next test to reuse the same rule_name
		# re-inserts a parent row with the SAME name and appends fresh child
		# rows on top of the previous test's orphaned ones still sitting in
		# the child table under that same parent name - e.g.
		# test_retrieve_eco_status seeing 4 required_disciplines instead of
		# 2 for "ECOSVC-TEST-Default" once an earlier test in this same
		# module run has already used and "cleaned up" that same rule_name.
		# Must be deleted explicitly here too.
		frappe.db.delete("Approval Matrix Discipline", {"parent": ["like", "ECOSVC-TEST-%"]})
		...
		# collides with this suite's own priority-100 "ECOSVC-TEST-Default"
